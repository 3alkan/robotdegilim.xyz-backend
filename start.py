...
def run_server():
    ...
    subprocess.run([python_bin, "-m", "uvicorn", "robotdegilim_xyz_backend.main:app", "--host", "0.0.0.0", "--port", "8000"])
...
def run_both():
    ...
    server_process = subprocess.Popen([python_bin, "-m", "uvicorn", "robotdegilim_xyz_backend.main:app", "--host", "0.0.0.0", "--port", "8000"])
